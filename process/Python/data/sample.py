...
from process.Python import SAMPLE_DATA_CFG
...
def simulation_sample_mortality(
    ...
    mortality_data['age_group'] = cut(mortality_data['age'], bins=bins, labels=labels, right=False)

    # Group by the new 'age_group' and 'ethnicity'
    # observed=False ensures you see all age groups even if counts are 0 (optional)
    mortality_data = mortality_data.groupby(['age_group', 'ethnicity'], observed=False)['deaths'].sum().reset_index()

    # Sort for better viewing (optional, but matches data.table structure)
    mortality_data = mortality_data.sort_values(by=['ethnicity', 'age_group']).reset_index(drop=True)
...
def generate_sample_population(n: int = 1000, seed_num: int = 42, hh_configs = SAMPLE_DATA_CFG["household_types"]) -> DataFrame:
    ...
    while current_n < n:
        ...
        hh_region = choice(['Auckland', 'Wellington', 'Christchurch', 'Others'], 
    ...
    if mask_young.any():
        df.loc[mask_young, 'ethnicity'] = choice(eth_cats, size=mask_young.sum(), p=[0.50, 0.25, 0.12, 0.10, 0.03])
        
    mask_adult = (df['age'] >= 20) & (df['age'] < 65)
    if mask_adult.any():
        df.loc[mask_adult, 'ethnicity'] = choice(eth_cats, size=mask_adult.sum(), p=[0.60, 0.15, 0.08, 0.14, 0.03])

    mask_old = df['age'] >= 65
    if mask_old.any():
        df.loc[mask_old, 'ethnicity'] = choice(eth_cats, size=mask_old.sum(), p=[0.85, 0.07, 0.03, 0.04, 0.01])

    # Education (Only relevant for 18+)
    df['education_level'] = 'None'
    mask_edu = df['age'] >= 18
    df.loc[mask_edu, 'education_level'] = choice(['Low', 'Medium', 'High'], size=mask_edu.sum(), p=[0.35, 0.40, 0.25])
    ...
    if mask_work.any():
        df.loc[mask_work, 'employment_status'] = choice(['Employed', 'Unemployed'], size=mask_work.sum(), p=[0.95, 0.05])
    ...
    hh_stats = df.groupby('household_id')['age'].apply(
        lambda x: Series({
    ...
    # Reorder for clarity
    cols = ['id', 'household_id', 'household_type', 'role', 'age', 'n_adults', 'n_seniors', 'n_children',
    ...
    return {"population": pop_data, "mortality": mortality_data}