...
def add_utility_err_to_tawa(
    ...
    df_merged = pd.merge(
        df_reform,
        df_sq.drop_duplicates(subset=join_keys),
        on=join_keys,
        how='left'
    )
...
def run_ruf_for_tawa(
    ...
    data_reform = pq_read_table(f"{output_dir}/utility_func_data_{filename_hash}_updated.parquet")
    data_reform = data_reform.to_pandas()
    data_reform = ruf_predict(data_reform, ruf_params, method=method, use_hhld= False)
    ...
    employment_scaler_data['employment_scaler'] = employment_scaler_data['hours_reform'] / employment_scaler_data['hours_sq']

    employment_num1 = len(employment_scaler_data[employment_scaler_data["employment_scaler"] > 1])
    employment_num2 = len(employment_scaler_data[employment_scaler_data["employment_scaler"] < 1])
    ...
    employment_scaler_data = employment_scaler_data.rename(
        columns={"household_id": "snz_hes_hhld_uid",
                 "people_id": "snz_hes_uid"}
    )

    tawa_data_input = tawa_data["input"].merge(
        employment_scaler_data,
        on = ["snz_hes_hhld_uid", "snz_hes_uid"],
        how = "left"
    )

    tawa_data_input['employment_scaler'] = tawa_data_input['employment_scaler'].fillna(1.0)

    for tawa_data_key in ["P_Income_SelfEmployed", "P_Income_WageSalary"]:
        tawa_data_input[tawa_data_key] = tawa_data_input[tawa_data_key] * tawa_data_input["employment_scaler"]
...
def tawa_data_preprocess(
    ...
    income_types: list = ["P_Income_Disposable"],
    ...
    # ---------------------------------------------
    df['P_Total_income'] = df[income_types].sum(axis=1) / 52.0
    ...
    #      we use minimum wage)
    df["income_per_hour"] = df["income_per_week"] / df["working_hours"]
    df['income_per_hour'] = df['income_per_hour'].clip(lower=min_hourly_wage)
    df["working_hours"] = df["income_per_week"] / df['income_per_hour'] 
