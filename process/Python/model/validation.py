...
def run_ruf_sensitivity(
    ...
    results = {"total_employment_hrs": [], "scaler": income_scaler}

    for scaler in income_scaler:
        logger.info(f"Processing sensitivity study for scaler: {scaler}")
        predicted_choices = ruf_predict(
            data_to_check, model_params, method=method, scaler=scaler
        )
...
def run_ruf_validation(
    input_params: dict, tawa_data_name: str, output_dir: str, method=RUF_METHOD
):
